...
impl WindowsRuntime {
    ...
    pub fn configure_app(&self, _app: &mut App) {
        ...
        log::info!(
            "Configuring Windows runtime: tray_support={} status_surface={}",
            tray_support,
            status_surface
        );
    }
    ...
    pub fn request_permissions(&self, _config: &mut Config) {
        log::info!(
            "Windows runtime does not request desktop permissions automatically; microphone and input injection access remain system-managed"
        );
    }
    ...
    fn log_sound_fallback(&self, sound_name: &str) {
        log::debug!(
            "Windows {} sound is disabled; no Windows sound backend is configured yet",
            sound_name
        );
    }
}
